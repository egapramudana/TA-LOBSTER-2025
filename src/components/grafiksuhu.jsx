// src/components/CanvasTemperatureChart.jsx
...
const CanvasTemperatureChart = ({ data = [] }) => {
  ...
  const chartData = data.length > 0 ? data : [{ x: 'No Data', y: 0 }];
  
  // Calculate min and max for y-axis
  const yValues = chartData.map(item => item.y);
  const minY = Math.floor(Math.min(...yValues, 18) - 2); // Min 18°C or lower with padding
  const maxY = Math.ceil(Math.max(...yValues, 32) + 2); // Max 32°C or higher with padding
  
  useEffect(() => {
    // Default to show the most recent data points
  ...
  useEffect(() => {
    // Set visible count based on mobile state
    setVisibleCount(isMobile ? 3 : 5);
  }, [isMobile]);
  
  useEffect(() => {
    // Handle resize to adjust visible count and canvas size
    const handleResize = () => {
  ...
  useEffect(() => {
    ...
    // Chart area dimensions with padding - add more bottom padding for x-axis labels
    ...
    for (let i = 0; i < yTickCount; i++) {
      ...
      ctx.fillText(yValue.toFixed(0), padding.left - 8, yPos);
    }
    
    // Reference lines for temperature ranges
    // Low temp threshold (20°C)
    const lowThresholdY = height - padding.bottom - (chartHeight * (20 - minY) / yRange);
    ctx.beginPath();
    ...
    // High temp threshold (30°C)
    const highThresholdY = height - padding.bottom - (chartHeight * (30 - minY) / yRange);
    ctx.beginPath();
    ...
    visibleData.forEach((point, index) => {
      ...
      ctx.fillStyle = colors.grey[100];
      ctx.font = isMobile ? '9px Arial' : '10px Arial'; // Smaller font
      
      // Draw text in two lines to save horizontal space
      const timeLines = displayTime.split(' '); // Split by space if there are multiple components
    ...
    // Legend text
    ctx.fillStyle = colors.grey[100];
    ctx.fillText(translate("temperature"), legendX + 20, legendY);
  ...
  const handleMouseMove = (e) => {
    ...
    for (const point of visibleData) {
      ...
      if (distance <= 15) { // Slightly larger than the dot for easier hovering
        ...
        {hoveredPoint && (
          ...
          >
            ...
            <Typography variant="body2" color={colors.greenAccent[500]}>
              {`${translate("temperature")}: ${hoveredPoint.y}°C`}
            </Typography>
          </Box>
        )}
      </Box>
...
export default CanvasTemperatureChart;  